...
import '../media.css';
      ...
      const handleInputQuant = event => {
             ...
             useEffect(
               ...
               const cream = { headers:
                ...
                axios.get('http://localhost:5000/users/orders', cream)
              .then(response => {
  ...
  const updateQuant = sing => {
        ...
        axios.put('http://localhost:5000/menu/update/quant', menuQuat, { headers:
          {
          "this-token": localStorage.getItem("this-token")
        }
      })
    ...
    const addToCart = e => {
      ...
      if (cart === '') {
       ...
       axios.post('http://localhost:5000/users/orders', moreOrder, { headers:
         {
         "this-token": localStorage.getItem("this-token")
       }
     })
    ...
    axios.put('http://localhost:5000/users/orders', moreOrder, { headers:
      {
      "this-token": localStorage.getItem("this-token")
    }
   })
